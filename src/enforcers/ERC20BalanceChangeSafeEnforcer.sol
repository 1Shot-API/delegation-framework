...
/**
 * @title ERC20BalanceChangeSafeEnforcer
 ...
 */
contract ERC20BalanceChangeSafeEnforcer is CaveatEnforcer {
    ////////////////////////////// State //////////////////////////////
        ...
        override
    {
        ...
        uint256 balance_ = IERC20(token_).balanceOf(recipient_);
        if (enforceDecrease_) { // we are preventing the balance from decreasing too much, we are not preventing increasing balance
            if (balanceCache[hashKey_] > balance_) { // no need to check in the scenario where the balance when up after execution
              require(balanceCache[hashKey_] - balance_ <= amount_, "ERC20BalanceChangeEnforcer:exceeded-balance-decrease");
            }
        } else {
