...
>
> Verification: impl + `TransparentUpgradeableProxy` + `ProxyAdmin` verified on Ethereum (1) and Base (8453) via Etherscan v2 (`forge verify-contract` with `--verifier custom --verifier-url "https://api.etherscan.io/v2/api?chainid=<n>"`). On Arc (5042), all three are verified on Blockscout PRO (chain id **5042**, not legacy `1243`): proxy and `ProxyAdmin` via Forge ([Foundry verification](https://docs.blockscout.com/devs/verification/foundry-verification)) with `--verifier blockscout`, `--verifier-url "https://api.blockscout.com/v2/api?chain_id=5042&apikey=$BLOCKSCOUT_API_KEY"`, and `--etherscan-api-key $BLOCKSCOUT_API_KEY` (embed `apikey` in the verifier URL; `--etherscan-api-key` alone on the URL without `apikey=` returns X402). The `LiFiSwapEnforcer` impl is verified via Blockscout PRO `action=verify_via_sourcify` ([Verification API](https://docs.blockscout.com/devs/verification/blockscout-smart-contract-verification-api)) after Sourcify metadata upload; `action=verifysourcecode` with standard JSON or flattened source still returns PRO `Internal server error` for this contract size. Link proxy → impl with `action=verifyproxycontract` on `0x29fcBBa…`. Existing signed delegations keep hitting v2 `0xD0e70cd777a527fB798e5EcA8800c5E3588041d4` on Base and continue to work; no migration required.

> **Arc mainnet (5042) — framework broadcast verification (2026-09-18):** Addresses are listed in [`deployments/arc-5042-manifest.json`](../deployments/arc-5042-manifest.json) (from `broadcast/*/5042/run-latest.json`; LiFi v3 proxy/impl/ProxyAdmin excluded — verified separately above). Re-run: `source .env && SOURCIFY_FALLBACK=1 ./script/verification/verify-arc-broadcast.sh`. Status check: `./script/verification/check-arc-verified.sh` (Blockscout PRO read API). Most caveat enforcers and `SimpleFactory` verify via Forge + Blockscout PRO. Large core contracts (`DelegationManager`, `MultiSigDeleGator`, `HybridDeleGator`) may return PRO `Internal server error` on direct `verifysourcecode`; use `SOURCIFY_FALLBACK=1` (Forge → Sourcify, then poll `verify_via_sourcify`). `HybridDeleGator` requires `--libraries lib/SCL/src/lib/libSCL_RIP7212.sol:SCL_RIP7212:0xCCD3B747…`. Sample `ERC1967Proxy` wallet `0xea2ad262…` is in the manifest; link impl with `verifyproxycontract` after impl verification. Not deployed on Arc per broadcast: v2 `LiFiSwapEnforcer`, `ChainlinkPriceRuleEnforcer`, `DelegationMetaSwapAdapter`.
